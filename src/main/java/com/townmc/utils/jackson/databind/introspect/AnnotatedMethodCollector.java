...
import com.townmc.utils.jackson.databind.introspect.AnnotatedMethodMap;
    ...
    extends com.townmc.utils.jackson.databind.introspect.CollectorBase
{
    ...
    public static com.townmc.utils.jackson.databind.introspect.AnnotatedMethodMap collectMethods(AnnotationIntrospector intr,
    ...
    com.townmc.utils.jackson.databind.introspect.AnnotatedMethodMap collect(TypeFactory typeFactory, com.townmc.utils.jackson.databind.introspect.TypeResolutionContext tc,
                                                                         JavaType mainType, List<JavaType> superTypes, Class<?> primaryMixIn)
    {
        Map<com.townmc.utils.jackson.databind.introspect.MemberKey,MethodBuilder> methods = new LinkedHashMap<>();
        ...
        for (JavaType type : superTypes) {
            ...
            _addMemberMethods(
                    new com.townmc.utils.jackson.databind.introspect.TypeResolutionContext.Basic(typeFactory, type.getBindings()),
        ...
        */
        ...
        // And then let's 
        if (methods.isEmpty()) {
            return new com.townmc.utils.jackson.databind.introspect.AnnotatedMethodMap();
        }
        Map<com.townmc.utils.jackson.databind.introspect.MemberKey, com.townmc.utils.jackson.databind.introspect.AnnotatedMethod> actual = new LinkedHashMap<>(methods.size());
        for (Map.Entry<com.townmc.utils.jackson.databind.introspect.MemberKey,MethodBuilder> entry : methods.entrySet()) {
            com.townmc.utils.jackson.databind.introspect.AnnotatedMethod am = entry.getValue().build();
            if (am != null) {
    ...
    private void _addMemberMethods(com.townmc.utils.jackson.databind.introspect.TypeResolutionContext tc,
                                   Class<?> cls, Map<com.townmc.utils.jackson.databind.introspect.MemberKey,MethodBuilder> methods, Class<?> mixInCls)
    {
        ...
        for (Method m : ClassUtil.getClassMethods(cls)) {
            ...
            final com.townmc.utils.jackson.databind.introspect.MemberKey key = new com.townmc.utils.jackson.databind.introspect.MemberKey(m);
            MethodBuilder b = methods.get(key);
            if (b == null) {
                com.townmc.utils.jackson.databind.introspect.AnnotationCollector c = (_intr == null) ? com.townmc.utils.jackson.databind.introspect.AnnotationCollector.emptyCollector()
...
//                } else if (old.getDeclaringClass().isInterface() && !m.getDeclaringClass().isInterface()) {
    ...
    protected void _addMethodMixIns(com.townmc.utils.jackson.databind.introspect.TypeResolutionContext tc, Class<?> targetClass,
                                    Map<com.townmc.utils.jackson.databind.introspect.MemberKey,MethodBuilder> methods, Class<?> mixInCls)
    {
        if (_intr == null) {
            return;
        }
        for (Class<?> mixin : ClassUtil.findRawSuperTypes(mixInCls, targetClass, true)) {
            for (Method m : ClassUtil.getDeclaredMethods(mixin)) {
                ...
                final com.townmc.utils.jackson.databind.introspect.MemberKey key = new MemberKey(m);
                MethodBuilder b = methods.get(key);
    ...
    private final static class MethodBuilder {
        ...
        public com.townmc.utils.jackson.databind.introspect.AnnotatedMethod build() {
            if (method == null) {
                return null;
            }
