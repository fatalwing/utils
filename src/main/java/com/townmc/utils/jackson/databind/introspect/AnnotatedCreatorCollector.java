...
import com.townmc.utils.jackson.databind.introspect.AnnotatedClass.Creators;
 ...
 * and static methods) are discovered to be accessed by and via {@link com.townmc.utils.jackson.databind.introspect.AnnotatedClass}.
 *
 * @since 2.9
 */
    ...
    extends com.townmc.utils.jackson.databind.introspect.CollectorBase
{
    ...
    private final com.townmc.utils.jackson.databind.introspect.TypeResolutionContext _typeContext;

    // // // Collected state

    private com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor _defaultConstructor;
...
//        TypeResolutionContext typeContext = new TypeResolutionContext.Basic(_typeFactory, _type.getBindings());
        ...
        List<com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor> constructors = _findPotentialConstructors(type, primaryMixIn);
        List<com.townmc.utils.jackson.databind.introspect.AnnotatedMethod> factories = _findPotentialFactories(type, primaryMixIn);
     ...
     * the two and construct resulting {@link com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor}s
     */
    private List<com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor> _findPotentialConstructors(JavaType type,
                                                                                                            Class<?> primaryMixIn)
    {
        ...
        List<com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor> result;
        int ctorCount;
        if (ctors == null) {
        ...
        if (primaryMixIn != null) {
            com.townmc.utils.jackson.databind.introspect.MemberKey[] ctorKeys = null;
            for (ClassUtil.Ctor mixinCtor : ClassUtil.getConstructors(primaryMixIn)) {
                ...
                if (ctors != null) {
                    if (ctorKeys == null) {
                        ctorKeys = new com.townmc.utils.jackson.databind.introspect.MemberKey[ctorCount];
                        for (int i = 0; i < ctorCount; ++i) {
                            ctorKeys[i] = new com.townmc.utils.jackson.databind.introspect.MemberKey(ctors.get(i).getConstructor());
                        }
                    }
                    com.townmc.utils.jackson.databind.introspect.MemberKey key = new com.townmc.utils.jackson.databind.introspect.MemberKey(mixinCtor.getConstructor());
        ...
        for (int i = 0; i < ctorCount; ++i) {
            com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor ctor = result.get(i);
            if (ctor == null) {
                result.set(i,
    ...
    private List<com.townmc.utils.jackson.databind.introspect.AnnotatedMethod> _findPotentialFactories(JavaType type, Class<?> primaryMixIn)
    {
        ...
        List<com.townmc.utils.jackson.databind.introspect.AnnotatedMethod> result = new ArrayList<>(factoryCount);
        for (int i = 0; i < factoryCount; ++i) {
            result.add(null);
        }
        // so far so good; but do we also need to find mix-ins overrides?
        if (primaryMixIn != null) {
            com.townmc.utils.jackson.databind.introspect.MemberKey[] methodKeys = null;
            for (Method mixinFactory : ClassUtil.getDeclaredMethods(primaryMixIn)) {
                if (!Modifier.isStatic(mixinFactory.getModifiers())) {
                    continue;
                }
                if (methodKeys == null) {
                    methodKeys = new com.townmc.utils.jackson.databind.introspect.MemberKey[factoryCount];
                    for (int i = 0; i < factoryCount; ++i) {
                        methodKeys[i] = new com.townmc.utils.jackson.databind.introspect.MemberKey(candidates.get(i));
                    }
                }
                com.townmc.utils.jackson.databind.introspect.MemberKey key = new MemberKey(mixinFactory);
                for (int i = 0; i < factoryCount; ++i) {
        ...
        // Ok: anything within mix-ins has been resolved; anything remaining we must resolve
        for (int i = 0; i < factoryCount; ++i) {
            com.townmc.utils.jackson.databind.introspect.AnnotatedMethod factory = result.get(i);
            if (factory == null) {
                result.set(i,
    ...
    protected com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor constructDefaultConstructor(ClassUtil.Ctor ctor,
                                                                                                         ClassUtil.Ctor mixin)
    {
        if (_intr == null) { // when annotation processing is disabled
            return new com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor(_typeContext, ctor.getConstructor(),
    ...
    protected com.townmc.utils.jackson.databind.introspect.AnnotatedConstructor constructNonDefaultConstructor(ClassUtil.Ctor ctor,
                                                                                                            ClassUtil.Ctor mixin)
    {
        ...
        com.townmc.utils.jackson.databind.introspect.AnnotationMap[] resolvedAnnotations;
...
ctor.getDeclaringClass().getName(), paramCount, paramAnns.length));
    ...
    protected com.townmc.utils.jackson.databind.introspect.AnnotatedMethod constructFactoryCreator(Method m, Method mixin)
    {
        final int paramCount = m.getParameterTypes().length;
        if (_intr == null) { // when annotation processing is disabled
            return new com.townmc.utils.jackson.databind.introspect.AnnotatedMethod(_typeContext, m, _emptyAnnotationMap(),
                    _emptyAnnotationMaps(paramCount));
        }
        if (paramCount == 0) { // common enough we can slightly optimize
            return new com.townmc.utils.jackson.databind.introspect.AnnotatedMethod(_typeContext, m, collectAnnotations(m, mixin),
                    NO_ANNOTATION_MAPS);
        }
    ...
    private com.townmc.utils.jackson.databind.introspect.AnnotationMap[] collectAnnotations(Annotation[][] mainAnns, Annotation[][] mixinAnns) {
        final int count = mainAnns.length;
        com.townmc.utils.jackson.databind.introspect.AnnotationMap[] result = new com.townmc.utils.jackson.databind.introspect.AnnotationMap[count];
        for (int i = 0; i < count; ++i) {
            com.townmc.utils.jackson.databind.introspect.AnnotationCollector c = collectAnnotations(com.townmc.utils.jackson.databind.introspect.AnnotationCollector.emptyCollector(),
                    mainAnns[i]);
            if (mixinAnns != null) {
    ...
    private com.townmc.utils.jackson.databind.introspect.AnnotationMap collectAnnotations(ClassUtil.Ctor main, ClassUtil.Ctor mixin) {
        com.townmc.utils.jackson.databind.introspect.AnnotationCollector c = collectAnnotations(main.getConstructor().getDeclaredAnnotations());
        if (mixin != null) {
